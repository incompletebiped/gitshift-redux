...
import { getCurrentGitUser, isGitRepository, getGitRemoteUrl } from './gitManager';
...
<body>
  ...
  private async _getHtmlContent(): Promise<string> {
    ...
    try {
      ...
      // Publish button in that state (the no-repo block above is hidden once the
      // folder is a repo, which would otherwise strand a freshly-initialized repo).
      if (isGitRepo) {
